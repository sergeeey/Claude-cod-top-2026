...
class TestIINRedaction:
    def test_valid_iin(self):
        result = redact("IIN: 920315450123")
        assert "[REDACTED:IIN]" in result
        assert "920315450123" not in result

    def test_iin_in_sentence(self):
        result = redact("Client IIN is 850101312345, verified.")
        assert "[REDACTED:IIN]" in result

    def test_short_number_not_iin(self):
        """11 digits should NOT match IIN pattern."""
        result = redact("92031545012")
        assert "[REDACTED:IIN]" not in result
...
class TestPhoneRedaction:
    ...
    def test_kz_phone_no_plus(self):
...
class TestClean:
    def test_clean_dict(self):
        data = {"name": "Test", "iin": "920315450123"}
        result = clean(data)
        assert "[REDACTED:IIN]" in result["iin"]
