...
def test_header_with_spaces_resolves_next_check(tmp_path):
    """Regression (found 2026-09-08 on the live N-7 registry): the registry's
    own header reads "Next check" (a space, title-cased) while every caller
    asks for "next_check" (an underscore, lowercase). Without normalising the
    header before indexing, _row_field looked up a key that was never in the
    map and returned "" for EVERY row -- 198/198 rows on the live registry
    were silently classified unanchored, and overdue detection never fired.
    The 2026-08-26 escaping work fixed the tokenizer while this, one function
    downstream, stayed broken and untested."""
    registry = tmp_path / "INDEX.md"
    registry.write_text(
        "| Date | Source | Observation | Prediction | Trigger | Next check | Status |\n"
        "|------|--------|-------------|------------|---------|------------|--------|\n"
        "| 2026-06-01 | exp-g22 | NCG SU(3)×U(1)_B-L | SUB-SM check | G99 | 2026-07-01 | pending |\n",
        encoding="utf-8",
    )
