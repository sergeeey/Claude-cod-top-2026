...
EVIDENCE_GATE:                          ← обязательный шаг перед вердиктом
  weakest_blocking_node: [атом + тип]
  если MODEL_OUTPUT или ASSUMPTION в blocking → FINAL_VERDICT не может быть Go
  если VERIFIED-SYNTHETIC в blocking → FINAL_VERDICT не может быть Go (только Conditional)

FINAL_VERDICT: [Go / Conditional / Stop]
  Go         — все blocking атомы VERIFIED-REAL + нет blocking contradictions
  Conditional — есть ASSUMPTION/SYNTHETIC или неразрешённое contradiction
  Stop        — blocking атом провален или MODEL_OUTPUT без независимой верификации
```
