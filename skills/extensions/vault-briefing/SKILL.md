...
description: >
  ...
  Режимы: /vault-briefing [проект], --brief (быстро), --deep (глубоко), --new (новый проект).
  НЕ для: поиска конкретного факта (используй Grep) — только для старта работы над проектом.
effort: medium
context: fork
allowed-tools:
  - Read
