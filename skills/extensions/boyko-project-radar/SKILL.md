...
description: >
  [STATUS: described] [CONFIDENCE: n=1 сессия, 2026-08-26 — первый реальный PROJECT-scan
  на этом же проекте нашёл живой баг (detection_expected drift) и 6 незарегистрированных
  хуков, но сам вердикт "МОЙ ВЫБОР" оказался построен на непроверенной [INFERRED]-находке
  сканера (см. HARD RULE 5) — оценка 6/10, узкое место методологии, не покрытия]
  Спрашивает ОДИН явный вопрос — Задача или Весь проект — прежде чем что-либо делать,
  затем реально прогоняет существующие сканеры (не гадает по дешёвым сигналам) и
  синтезирует находки в 4 обязательные категории: узкие места, сильные места,
  нерешённые места, слепые места. Заканчивается рекомендацией цепочки скиллов под
  именно эти находки — в формате /suggest, но на реальных данных, не на git status.
  ...
  исследовательский), sci-code-audit (доверие к коду, если есть в каталоге).
