...
| 20260716-response-guard-fp-calibration | 2026-08-28 | response-guard-fp-calibration | RFC-003 severity_calibrator.py (steps 1-5) real and shadow-wired, but step 6 got only an n=4 probe (2026-07-16) and no further shadow data since — stalled untracked for 6 weeks (found by research-audit + boyko-project-radar, 2026-08-28); defect it measured (8/13 FP, 2/12 FN) stays honestly unsolved (xfail tests still red), not fixed, not regressed | `CLAUDE_GUARD_SHADOW=1` collects a real shadow sample past n=30, OR a new FP/FN incident makes the cost acute, OR a genuinely different mechanism is proposed (not a 3rd regex/LLM-judge variant — both already REJECTed separately) |
