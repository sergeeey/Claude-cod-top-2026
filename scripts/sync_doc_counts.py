...
def wiring_counts() -> dict[str, int]:
    """Wired/dormant/library/prevent breakdown, reused directly from
    gen_hook_matrix.py's own classify_wiring()/classify_capability() -- NOT
    reimplemented, so this can never disagree with docs/hook-control-matrix.md
    (which is generated by that same module) the way three hand-written prose
    sentences already had, independently of each other, before this function
    existed (skeptic-flagged, 2026-09-13, in the same review that found the
    "N_guards" badge overclaim this file's anchor list already fixes):

      README.md's "## 101 Hooks" intro said "83 of the 95 are wired; 6 are
      dormant" against a then-true "93 wired · 2 dormant · 6 library" --
      docs/architecture.md's Layer 5 section said "only 7 of the 95 can
      actually deny a tool call" against a then-true 8 (6 PREVENT + 2
      PREVENT-on-PreToolUse-leg-only) -- and README's second hooks badge
      title said "88 wired" against the same then-true 93. Three different
      wrong numbers for the same two underlying quantities, each hand-copied
      once and never revisited. Filesystem-authoritative like every other
      function in this file; kept separate from actual_counts() for the same
      reason event_count() is (see that function's docstring and
      actual_counts()'s own) -- these are not part of the ci.yml bash-loop's
      6-key invariant.
    """
    registry_path = REPO / "hooks" / "registry.yaml"
    settings_path = REPO / "hooks" / "settings.json"
    if not registry_path.exists() or not settings_path.exists():
        return {"wired": 0, "dormant": 0, "library": 0, "prevent": 0}  # minimal test fixtures
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    from gen_hook_matrix import (
        _PREVENT_CAPABILITIES,
        classify_capability,
        classify_wiring,
        parse_registry,
        parse_wired,
    ...
    wired = parse_wired(settings_path.read_text(encoding="utf-8"))
    counts = {"wired": 0, "dormant": 0, "library": 0, "prevent": 0}
    for name, fields in entries.items():
        wiring = classify_wiring(name, fields, wired)
        if wiring in counts:
            counts[wiring] += 1
        # WHY imported, not re-literalled (skeptic finding, round 2, 2026-09-13): an
        # earlier version of this line duplicated {"PREVENT", "PREVENT (on PreToolUse
        # leg only)"} as its own literal set -- if gen_hook_matrix.classify_capability()
        # ever grows a third PREVENT-shaped label, that copy would silently stop
        # counting it while docs/hook-control-matrix.md's own capability_counts picked
        # it up automatically, reproducing this exact file's founding bug one level up.
        if classify_capability(fields, wiring) in _PREVENT_CAPABILITIES:
            counts["prevent"] += 1
    return counts
...
_ANCHORS: list[tuple[str, str, tuple[str | None, ...]]] = [
    ...
    # WHY these 3 (skeptic-flagged, 2026-09-13): the "## 101 Hooks" intro
    # paragraph, README's second hooks badge title, and architecture.md's
    # Layer 5 section each hand-copied wired/dormant/library/prevent counts
    # once and never revisited -- three DIFFERENT wrong numbers found for the
    # same two underlying quantities in one pass. See wiring_counts()'s own
    # docstring for the exact before/after numbers.
    (
        "README.md",
        r"(\d+)( of the )(\d+)( are wired; )(\d+)( are dormant)",
        ("wired", None, "hooks", None, "dormant", None),
    ),
    (
        "README.md",
        r'(title=")(\d+)( wired, )(\d+)( dormant, )(\d+)( internal library modules)',
        (None, "wired", None, "dormant", None, "library", None),
    ),
    (
        "docs/architecture.md",
        r"(only )(\d+)( of the )(\d+)( can actually deny a tool call)",
        (None, "prevent", None, "hooks", None),
    ),
    (
        "README.md",
    ...
    # WHY "defined" not "always_on"/"guards" (external audit 2026-08-14, sharpened
    # 2026-09-13 after a second, independent audit found the SAME failure mode one
    # word over): some are `class: dormant`, some are `class: library` (imported by
    # other hooks, never themselves triggered), and of the rest that ARE wired, most
    # are OBSERVE/WARN, not PREVENT -- "guards" (a word this file's own two README
    # anchors used to carry, both fixed in the same pass as this comment) implies
    # uniform blocking capability none of that has. The exact wired/dormant/library
    # split, and the OBSERVE/WARN/PREVENT breakdown of the wired ones, DRIFTS as
    # hooks are added -- do not hardcode either count here; read
    # docs/hook-control-matrix.md's own "Totals" line for the live numbers. "defined"
    # is the one claim true of every entry without exception: each is a registered
    # hook definition in hooks/registry.yaml, regardless of wiring or class.
...
def main() -> int:
    ...
    actual.update(wiring_counts())  # same reasoning -- wired/dormant/library/prevent
    print(
