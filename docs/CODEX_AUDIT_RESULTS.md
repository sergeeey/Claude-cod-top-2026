...
- **~15 broken skill-to-skill references** (`/lit-search`, `/evolve-solution`, `/orient`, `/stat-validate`, `research-strategist`, `validate-blind`, and others) — names that don't exist as skill directories. A few of these (per the false-positive check above) may actually be agent references worded ambiguously; each needs a one-line `ls` check before fixing.
  ...
  - `/lit-search` — **same policy, NOT yet applied — deliberately deferred.** Same portability class, but far bigger footprint: ~15 references across `rules/falsification-ladder.md`, `docs/strategy-router.md`, and 4 skill files (`boyko-specialist`, `hypothesis-revival`, `paper-assembly`, `research-pipeline`), woven into disambiguation tables as the canonical literature-search delegate. Removing all of them needs its own deliberate pass (or a decision to special-case it), not a mechanical strip alongside the two cheap ones above.
