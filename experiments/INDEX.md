...
|---|---|---|---|---|
| 20260912-cycle2-retrospective-replay | 2026-09-12 | do the Cycle-1 mechanisms flag anything when replayed over this repository's own verdict history? | Full | REPEAT (`NO_HEADROOM` for the mechanism as shipped: floor and Cycle 1 both fire 0/12; the 3 Layer-B firings come from a parser Cycle 1 did not ship. Four findings, two of them defects in Cycle 1's own output. Causal question ruled non-identifiable at the L0 gate — PR [#446](https://github.com/sergeeey/Claude-cod-top-2026/pull/446)) |
| 20260912-research-evidence-loop-minimal-extension | 2026-09-12 | Claim/Experiment DAG + Sealed Holdout + independence detail + OBSERVE-only priority score, as minimal opt-in extensions rather than a new framework | Full | NEEDS-MORE-DATA (infrastructure verified sound and non-regressive; whether it changes real outcomes is Cycle 2's question. PR [#444](https://github.com/sergeeey/Claude-cod-top-2026/pull/444)) |
| 20260903-memory-retrieval-repair | 2026-09-03 | corpus fingerprint + `glob`→`rglob` repair the memory-retrieval path (PR-1 of a 7-PR Full Ladder, see `docs/memory-retrieval-repair-tz.md`) | Full | PROMOTE (claim holds; merged to main) |
| 20260728-hypothesis-arbiter-taxonomy-pilot | 2026-07-28 | adding the 8-class generator taxonomy to `hypothesis-arbiter`'s SPAWN step causes the candidate table to include an Artifact-class hypothesis | Full | Filing status: ARCHIVE → `parked/20260728-hypothesis-arbiter-taxonomy-pilot.md`; claim-level result: REJECT (falsified as stated). Two verdicts at different levels — see `docs/experiment-dependency-graph.md` § Known limitation |
| 2026-07-16-route-trace-sci-hypothesis | 2026-07-16 | the `scientific-hypothesis` route is machine-connected end-to-end: every capability resolves to a real skill, a verifier gates the result, and the outcome has a memory sink and a failure sink | — | Trace artifact only (`README.md` + `route.json`, no `decision.md`) — executable architectural-coherence evidence, not a claim under test. Note: non-standard id format (`YYYY-MM-DD-` rather than `YYYYMMDD-`) |
