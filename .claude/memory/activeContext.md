...
- **Done when:** install.sh работает на trёх машинах, CI зелёный, coverage ≥ 65% (CI/Linux)
...
## Current Focus
Learning loop restored ✅. Next: install.sh на второй машине (sboi / D:\).



## What was done this session
- **Learning loop restored** [2026-04-17] — knowledge_librarian.py + auto_capture.py вписаны в ~/.claude/settings.json
- **hook_main added** — в глобальный ~/.claude/hooks/utils.py (был только в проектном utils.py)
- **input_guard fix** — backtick pattern сужен до shell-context, markdown inline code больше не блокируется
- **61 eval tests** (`tests/test_skill_routing.py`) — routing-policy signal matrix + skill file integrity [PR #71 ✅]
- **Skill descriptions** — NOT-for clauses + WHY rationale to routing-policy/tdd-workflow/security-audit [PR #71 ✅]
- **dry-run mode** (`CLAUDE_DRY_RUN=1`) — session_save.py + auto_capture.py [PR #71 ✅]
- **wiki_reminder evidence check** — `_has_verified_evidence()`, No Execution No Memory pattern [PR #72]
...
## Recent Merges
- #68 feat: observation_capture + /ship skill + retro metrics — 862 tests, claude-mem/gstack patterns
...
[summarized] [summarized] [summarized] [summarized] [summarized] [summarized] [summarized] [summarized] - **Social Engineering Guard:...
