...
**2 items remain OPEN, both need YOUR explicit decision (not mine to make unilaterally):**
...
2. `hooks/iteration_guard.py` cap=3 enforcement — the 4th reviewer↔builder cycle currently only gets extra context, not an actual block, despite CLAUDE.md documenting a hard cap. Same shape of decision: blocking vs. escalation-only.
