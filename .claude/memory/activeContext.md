...
| **next action** | [VERIFIED] Full 10-task B6 Strong Inference benchmark completed this session, per user's explicit "запусти полный корпус 8-12 задач" instruction (`benchmarks/strong-inference/run-2026-07-23-full.md`, merged to main). All 5 §14 population categories covered; all 10 tasks blind-graded by an independently tool-verifying `verifier` agent. **Results:** strict full-correct count Arm A (plain) = 6/10, Arm B (`hypothesis-arbiter`) = **10/10**, Arm C (deep-spec 12-step, literal) = 7/10. **MCID MET.** `hypothesis-arbiter.maturity` promoted `wired` → **`dogfooded`** in `skills/registry.yaml`, `maturity_evidence` citing this run. Full pytest (2450 passed), ruff, check_architecture clean before merge. **Then, per user request, the §14 sensitivity check (shuffled candidate-hypothesis order) was completed to full 10/10-task coverage** (`6eb3577` for the first 8, then Tasks 3/4's exact original prompts were recovered from the raw session transcript JSONL via targeted `Grep` after initially being thought lost to context compaction, closing out the last 2 tasks). **Final tally: 16/20 (Arm B × Arm C × 10 tasks) stable, 4 flipped.** Corrected an earlier (8-task-partial) finding: Arm B is **9/10 stable, not perfectly stable** — its one miss (Task 3) is the same wrong-commit citation (`0e75a70` instead of `d316d1c`) that cost the original Arm C its grade, traced to `aecd738`'s own commit *message* misattributing the rename even though that commit's *diff* doesn't touch the file — a shared repo-level trap (an agent trusting commit-message prose over the actual diff), not an arm-specific weakness; both arms independently fell into it on different runs. Arm C: 7/10 stable, 3 flipped (2 improved — Task 3 and Task 7 citation/wording precision gains; 1 regressed — Task 1's "confident wrong-target" mechanism-drift, where live tool access let the agent substitute a real-but-off-topic mechanism for the asked question with high confidence, a distinct risk from hedging), net **+0.5** strict-count, directionally favorable to Arm C. MCID and `dogfooded` maturity verdicts unaffected (rest on the original 10-task run, not this separate check). **Inter-rater agreement started per user request**: 10-item stratified sample (all 4 sensitivity-check flips + 6 stable) re-graded by a genuinely independent second blind grader (no info about grader-1's verdict). **Result: 80% (8/10) agreement**, both disagreements boundary cases (CORRECT vs PARTIALLY, PARTIALLY vs INCORRECT — never a flat contradiction), informative about a real completeness-bar ambiguity in the grading rubric, not grader carelessness. Task 1 Arm C needed 3 attempts (2 background agents returned empty results — disclosed as an infrastructure hiccup per Substrate Gate discipline, not evidence either way). **Then extended to the original 30-item population per explicit user request** ("run the full 30-item inter-rater check"): recovered original answer text for 24/30 items (Tasks 3-10, all 3 arms) from the raw transcript JSONL via targeted `Grep`; Tasks 1-2 (6 items) honestly excluded — their original text predates the recoverable transcript region, multiple grep passes failed. **Sample 2 result: 22/24 = 91.7% agreement**, both disagreements the same boundary-case shape as Sample 1's (grader 2 slightly more generous on completeness-bar questions, never a flat CORRECT-vs-INCORRECT contradiction). **Combined across both samples: 32/34 = 94.1% agreement, covering 24/30 (80%) of the original population** the MCID/`dogfooded` claims rest on. Still open toward a full `benchmarked`-tier claim: the 6 unrecoverable items and no formal chance-corrected statistic (Cohen's kappa) — disclosed as real gaps, not closed. |
