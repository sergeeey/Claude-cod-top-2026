...
| **branch** | `fix/sec-01-remove-code-runner-auto-allow` (commit `70c5459`), pushed, PR #205 open. Branched off `main` @ `1160187` (PR #204 + PR #201 both merged). |
| **last verified SHA** | `70c5459` — local full suite 2255 passed (env-dependent; CI's own Ubuntu run measured 2243 for this branch's test-count badge, README synced to that number, not the local one). ruff/mypy clean. First CI run on PR #205 caught this exact drift (README said 2237, CI's own count was 2243) — fixed in a follow-up commit same PR. |
...
| **current focus** | User received a full external comprehensive/security audit report (SHA `17862da`, pre-dating PR #201's merge) and asked me to re-verify it rather than trust it. Independently confirmed all 7 findings true against actual code (0 false positives — unusually clean hit-rate vs. my own earlier 2-pass skeptic audit, which had 1 false alarm): SEC-01 (HIGH, pytest/npm test auto-allowed via prefix match — executes repo-defined code like conftest.py/package.json scripts with zero confirmation), AI-01 (MEDIUM, `Task` in `ALWAYS_SAFE_TOOLS`, no capability scoping on subagent delegation), AI-02 (MEDIUM, `validation_theater_guard` is `PostToolUse` — post-hoc, can't prevent the effect, plus skips on `CLAUDE_INVOKED_BY`), SEC-02 (MEDIUM, `webhook_notify.py` DNS-resolution-failure fails open + separate re-resolution at connect time = TOCTOU), RUN-01 (MEDIUM, `session_save.py`'s `file_lock` only covers the daily-note path, not the general `activeContext.md` read-modify-write — lost-update race possible), SUPPLY-01 (MEDIUM, `update-claude.sh` is just `git pull origin main` + `install.sh`, zero signature/checksum/rollback), DOC-01 (LOW, found *another* stale metric — "1200+ tests" — beyond what my own audit pass caught). Fixed SEC-01 (the HIGH) same session: removed `pytest`/`python -m pytest`/`npm test`/`npm run test`/`npm run lint` from `permission_policy.py`'s `SAFE_BASH_PREFIXES` (kept `ruff`/`mypy` — pure static analyzers, don't execute checked code); replaced 2 tests that had codified the old auto-allow as correct with 8 new regression tests (`TestDecideCodeRunnersRequireConfirmation`), including a `pytest-malicious` lookalike-collision case. Also fixed a self-caused structural bug mid-edit: the new test class got inserted into the middle of the existing `TestDecideSensitivePathRead` class, orphaning its later tests — caught and corrected before commit. Also fixed the DOC-01 "1200+ tests" stale line and reworded an unfalsifiable "every mistake never repeats" marketing line. The other 5 MEDIUM findings (AI-01, AI-02, SEC-02, RUN-01, SUPPLY-01) are real but NOT fixed — each needs real engineering (capability-scoped subagent tokens, pre-effect validation redesign, DNS-pinned egress, transactional state locking, signed releases), not a doc/config tweak, and were explicitly left as the user's own risk-appetite decision rather than silently changed. |
| **blockers** | none |
| **next action** | Push `fix/sec-01-remove-code-runner-auto-allow`, open PR, get CI green, merge — before whatever happens with the external audit next. The 5 remaining MEDIUM findings are an open decision, not a task: user needs to decide whether to fix before audit handoff or document as known limitations (repo already does this honestly elsewhere, e.g. "not independently verified", "single-developer"). ~600 orphaned `git.exe` processes on this machine (IDE git-integration leak) — flagged, unrelated to the repo, not fixed. |
