...
**1 item remains OPEN, needs YOUR explicit decision (not mine to make unilaterally):**
1. `hooks/promotion_gate_guard.py:198` — a `[x] PROMOTE` with failed conditions still gets persisted (hook is PostToolUse, can only warn after the write, not block it). Fixing this for real means adding a PreToolUse companion hook that denies the write before it happens — a behavior change from advisory to blocking. Needs your call on whether that tradeoff is wanted.

**RESOLVED (2026-07-07, later):** `hooks/iteration_guard.py` cap=3 — user explicitly decided "should block, not just warn." Implemented: the hook now also fires on `PreToolUse(Agent)` and denies (`permissionDecision: deny`) any `Agent(subagent_type=reviewer|builder)` call while the per-session counter is already `>= CAP=3`. Verified against Claude Code's official tools-reference docs (via claude-code-guide agent) that PreToolUse deny genuinely applies to the Agent tool type, not just Bash/Edit/Write, before building on that assumption. Registered a new `"matcher": "Agent"` block in `hooks/settings.json` under `PreToolUse`. Gate resets on an LGTM verdict (not a permanent block); other subagent types (explorer/tester/navigator/skeptic/...) are never blocked. 11 new regression tests added, all passing; caught and fixed 2 real bugs of my own while writing them: (a) forgot `sys.exit(0)` after the deny print — deny JSON was emitted correctly but the hook then fell through to `main()`'s implicit return instead of exiting, which pytest's `SystemExit` check correctly caught; (b) several tests set up `HookState` before `monkeypatch.chdir(tmp_path)` — `HookState.__init__` captures `Path.cwd()` at construction time, so those writes silently landed in the wrong directory (tests still passed, but for the wrong reason — count defaulted to 0 either way). Full suite: 1953 passed, 0 failed (the date-boundary flake cleared). Reviewer-agent pass in progress before commit. Found agent_context_filter.py has the exact same "claims PreToolUse(Agent) in its docstring but never registered in settings.json" gap as 11 other files closed earlier tonight — flagged via spawn_task (task_f51e704f) rather than fixed in this pass (out of scope for this specific request).
