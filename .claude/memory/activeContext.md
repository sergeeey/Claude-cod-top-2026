...
| **hooks / agents / skills** | [INFERRED] **97** / 13 / 129 (2026-08-23: +2 hooks `independence_scorer.py` + `mutation_tracker.py` on branch `feat/elai-independence-mdr`, not yet merged — count will be confirmed by `sync_doc_counts.py` on merge). Prior verified: 95/13/129 via `scripts/sync_doc_counts.py --check` 2026-07-24. |
...
[summarized] Protocol). Нашёл 4 реальных новых куска, подтверждено отсутствие через grep:
...
- [2026-08-22 19:37] `7158b30`: docs: regenerate hook-control-matrix.md via gen_hook_matrix.py
- [2026-08-22 19:30] `401f834`: fix(hooks): remove markitdown_auto_convert from registry.yaml, sync doc counts
- [2026-08-22 19:18] `41593fb`: fix(tests): update test_runs_without_crashing assertion to check errors=0 only
