...
FIXED (83ecf4f, branch fix/readme-metrics-drift, NOT yet merged/pushed as of this note): updated all 7 stale mentions (2 badges + 5 text occurrences) to 1681/76%/86. Verified locally by running the exact same extraction logic the CI job uses — now matches. mypy also run for the first time this session (exit 0, not the failure cause). NEXT IMMEDIATE STEP: merge this branch to main and push again, then re-check GitHub Actions status on the new commit to confirm test(3.11)/test(3.12) go green.
