...
| **next action** | None blocking — infra PR queue, P1/P2 backlog, and the two adoptable orchestration-doc gaps are all closed. Per DEFAULT FOCUS BIAS above, next unprompted work should shift toward §8 experimental packs unless the owner directs otherwise. Open, non-urgent: the stale `ANTHROPIC_API_KEY` itself (owner's credential fix); whether to arm `/release-scout` on a weekly schedule (owner decision, not yet asked); `claude-md/CLAUDE.md`'s RULES bullet list is missing 6 real `rules/*.md` files (`estimand-ops.md`, `evidence-markers.md`, `falsification-ladder.md`, `perelman-audit.md`, `research-methodology.md`, `skeptic-triggers.md`) — confirmed real by the #317 reviewer via `Glob`, deliberately left unfixed (out of scope for that PR), flagged here so it isn't lost. |
