...
## Current Focus
**PUSHED + RF-01 FIXED (2026-07-07 ~11:00):** all local security-fix commits from tonight (fa97c2a...3d26564, 24 commits) pushed to `origin/fix/pre-compact-dedup` (fast-forward, cd22ac1..3d26564). PR from fix/pre-compact-dedup → main NOT yet opened — `gh` is unauthenticated in this environment (`401: Bad credentials`), user given the manual compare URL and the option to `gh auth login` themselves. External re-audit (WebFetch-based, reading raw GitHub) correctly flagged that local work wasn't visible until pushed, then correctly found one new residual gap after the push: RF-01 — `hooks/session_start.py`'s `_TRUST_CRITICAL_PREFIXES`/`_TRUST_CRITICAL_FILES` only covered agent-behavior paths (hooks/agents/commands/skills/rules/CLAUDE.md), missing the CI/installer/MCP surface. Verified real against actual repo layout (all suggested paths exist) before fixing. Expanded to add `.claude/commands/`, `scripts/`, `mcp-profiles/`, `.github/workflows/`, `claude-md/` prefixes + `install.sh`, `install.ps1`, `update-claude.sh`, `skill-manager.sh`, `requirements.txt`, `pyproject.toml` files. 11 new parametrized tests; fixed 3 pre-existing tests whose "non-trust-critical" fixture (`scripts/build.py`) became trust-critical (swapped to `docs/README.md`, preserving original intent). Full suite: 2000 passed, ruff clean. Committed 3d26564, pushed. Reviewer-agent pass on 3d26564 escalated after 3rd non-LGTM cycle (evaluator-optimizer cap, per autonomy-budget guard) instead of looping a 4th time — surfaced 1 real P1 + 1 P2 for direct fix rather than another review round. P1 (verified via `git ls-files ".claude/<name>/"`): this repo git-tracks small `.claude/hooks/`, `.claude/agents/`, `.claude/rules/`, `.claude/scripts/`, `.claude/skills/` dirs (example/template files) — same trust-critical content as top-level namesakes, but didn't match the bare prefixes (same gap `.claude/commands/` in 3d26564 already closed, just not extended to the other five). P2: printed warning message hardcoded the OLD 6-path list in user-facing text, stale after 3d26564 grew the set to ~20 entries — replaced with a reference to the source-of-truth constant names so it can't drift again. Fixed both in d70d5b2: 5 new .claude/<name>/ prefixes + dynamic message text, 5 new parametrized test cases (16 total). Full suite: 2005 passed, ruff clean. Committed + pushed (d70d5b2).

**PR #170 OPENED (2026-07-07 ~12:00):** https://github.com/sergeeey/Claude-cod-top-2026/pull/170, fix/pre-compact-dedup → main. `gh` blocked by an invalid `GH_TOKEN` env var shadowing a valid keyring session — `env -u GH_TOKEN gh pr create` worked. External audit then reported CI checks failing (test 3.11/3.12, ~13-15s each — too fast to be the real suite). Pulled the actual job log (`gh run view --log`) instead of guessing: `mypy --ignore-missing-imports hooks/ scripts/` failed with 4 errors in hooks/weakened_test_guard.py (HookState.get() returns `object`, narrowed with `cast(dict, ...)`) and hooks/promotion_gate_guard.py (tool_input.get() on untyped dict returns Any into a `-> str` function, wrapped in `str(...)`). Verified via `git checkout origin/main -- <files>` that main's versions are mypy-clean — these were regressions introduced earlier on THIS branch (6698f70/2d27117, 32077e1/135a071/8700063), not by today's 3 session-specific fixes, but blocking regardless. **Lesson applied against self:** I had only run ruff+pytest all session, never mypy — exactly this repo's own "verified subset, claimed whole" [AVOID×4] pattern. Fixed + committed (e3f98e0), pushed. Also proactively checked (before pushing) the other CI gates that mypy had been blocking from ever running: doc-count/README-pattern/plugin.json/marketplace.json counts (85 hooks/15 agents/119 skills/14 rules/12 core/107 ext) all already match filesystem — no drift. Registry↔disk consistency: 0 orphans (ghosts are warn-only, pre-existing). **Found but NOT yet fixed:** README Tests badge says 1730, actual local count is 2005 — this branch accumulated ~275 tests across many commits without ever re-syncing the badge (scripts/sync_readme_from_ci.py only pulls from `main`'s latest run, which doesn't reflect this branch — can't use it as-is). Plan: let the just-pushed CI run complete, read its own "Actual: NNNN tests, MM% coverage" line from the verify-metrics step, and sync README to that exact number (same method the script encodes, just against this PR's own run instead of main's). **Still pending:** wait for CI run 28854264009 to finish, then fix the README badge from its authoritative count, then confirm PR is fully green.
