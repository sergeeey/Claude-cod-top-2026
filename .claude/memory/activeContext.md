...
| **branch** | `fix/sec-02-webhook-dns-pinning-fail-closed` (commit `8a2bc5d`), pushed, PR #206 open. PR #205 (SEC-01) merged first (`61883d2`), this branch rebased cleanly onto it, no conflicts. |
| **last verified SHA** | `8a2bc5d` — local full suite 2260 passed, ruff/mypy clean. First CI run on PR #206 hit the same class of drift as PR #205: 5 new tests raised the count CI measures (2243→2248), README badge was stale — fixed in a follow-up commit same PR, same pattern as every prior PR this session. |
...
| **current focus** | Continuing the external-audit response: user asked which of the diagram's "improvement suggestions" to actually implement. Gave a differentiated recommendation rather than blanket-accepting the list: SEC-02 (DNS pinning) — worth doing, contained/opt-in feature; RUN-01 (shared locking) — worth doing partially (extend existing `file_lock`, not a full rewrite); SUPPLY-01 (signed releases) — worth doing partially (pin updates to a tag by default, not full signing infra); AI-02 ("move PostToolUse checks to PreToolUse") — rejected as a category error, `validation_theater_guard` audits a claim that doesn't exist until after the tool ran; AI-01 (Task capability scoping) — flagged as overstated, since Claude Code's hooks are global-per-session, so a subagent's own Bash/tool calls already re-pass through the same `permission_policy.py` check. User asked me to do SEC-02. Fixed: replaced `_resolves_to_blocked_ip(hostname) -> bool` (fail-open on DNS failure) with `_resolve_safe_ip(hostname) -> str | None` (fail-closed, returns the validated IP itself). `send_webhook()` now resolves once immediately before connecting and monkeypatches `socket.getaddrinfo` (scoped by try/finally) so the connection uses exactly the validated IP, closing the DNS-rebinding TOCTOU window between the earlier `get_webhook_url()` validation and the actual `urlopen()` connect. `_ValidatingRedirectHandler` extends the same pin to redirect targets via a shared `pins` dict. Chose a monkeypatched-`getaddrinfo` pin over hand-rolled `HTTPConnection`/`HTTPSConnection` subclasses — same effect, much less code, TLS/SNI/cert-hostname verification untouched since only address resolution is intercepted. Added `TestSendWebhookDnsPinning` (5 new tests, fully deterministic — no real DNS/network) + fixed the existing fail-open test to fail-closed + updated `_ValidatingRedirectHandler`'s tests for its new `pins` constructor arg. mypy caught one real type error (`info[4][0]` is `str \| int` per stdlib stubs) — fixed with an explicit `str()`. |
| **blockers** | none |
| **next action** | Push `fix/sec-02-webhook-dns-pinning-fail-closed`, open PR, get CI green, merge. Remaining open items unchanged: RUN-01 and SUPPLY-01 partial fixes (user's call whether to do now or later), AI-01/AI-02 explicitly NOT going to be "fixed" as literally suggested by the diagram (see current focus) — worth documenting as known limitations instead. ~600 orphaned `git.exe` processes on this machine (IDE git-integration leak) — flagged, unrelated to the repo, not fixed. |
